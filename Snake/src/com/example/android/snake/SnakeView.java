...
import com.example.android.snake.Dir;
import com.example.android.snake.Coordinate;
import com.example.android.snake.Commander;
...
public class SnakeView extends TileView {
    ...
    private void initSnakeView() {
		...
		dpadControl.move(140-dpadControl.getWidth()/2, 440-dpadControl.getHeight()/2);
	...
	public boolean onTouchEvent(MotionEvent event) {
    	float x = event.getX();
    	float y = event.getY();
    	Control dapControl = mControls.get(0);
    	x = (x - dapControl.getX()) / dapControl.getWidth();
		y = (y - dapControl.getY()) / dapControl.getHeight();
    	int dpadState = getDpadStates(x, y);
    	if (dpadState == Keycodes.GAMEPAD_UP) {
            ...
            if (mDirection != Dir.DOWN) {
                mNextDirection = Dir.UP;
            }
            return (true);
        }

        if (dpadState == Keycodes.GAMEPAD_DOWN) {
            if (mDirection != Dir.UP) {
        ...
        if (dpadState == Keycodes.GAMEPAD_LEFT) {
            if (mDirection != Dir.RIGHT) {
                mNextDirection = Dir.LEFT;
            }
            return (true);
        }

        if (dpadState == Keycodes.GAMEPAD_RIGHT) {
            if (mDirection != Dir.LEFT) {
    ...
    public void setMode(int newMode) {
        ...
        mStatusText.post(new Runnable() {
			...
			public void run() {
				int newMode = SnakeView.this.mMode;
    ...
    public void update() {
        if (mMode == RUNNING) {
            ...
            if(g_autorun)
            {
            	SnakeView.this.setNextDirection(mCommander.getNextDir(mSnakeTrail, mAppleList, SnakeView.this.getDirection()));
            }
            
            if(mSnakeTrail.get(0).y==1 && mSnakeTrail.get(0).x==1)
            {
            	int x;
            	x = 1;
            }
